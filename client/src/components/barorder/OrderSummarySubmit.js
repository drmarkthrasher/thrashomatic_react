...
const OrderSummarySubmit = (props) => {
  const { device, server, service } = useContext(GlobalContext);

  const [liquors, setLiquors] = useState(props.location.state.liquors);
  ...
  return (
    <div>
      ...
      <p>Summary Page ID: </p>
      <Link to='/liquorbarmenu'>Back to Bar</Link>
      <p></p>
      <Button
        color='primary'
        variant='outlined'
      ...
      >
        {submitButtonReady ? 'Submit Drink' : 'Not Ready'}
      </Button>
    </div>
  );
};

export default OrderSummarySubmit;
